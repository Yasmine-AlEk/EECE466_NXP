...
class GaussianPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.log_std = nn.Parameter(torch.tensor([-2.0, -2.3]))
    ...
    def _scale_action(self, raw: torch.Tensor) -> torch.Tensor:
        ...
        # Zero-centered asymmetric scaling:
        # raw_speed = 0 -> delta_speed = 0
        # raw_speed > 0 -> up to +speed_up
        # raw_speed < 0 -> down to -speed_dn
        speed = torch.where(
            speed_raw >= 0.0,
    ...
    def entropy(self) -> torch.Tensor:
        ...
        two_pi = torch.tensor(2.0 * torch.pi, device=self.log_std.device)
        return (self.log_std + 0.5 * (1.0 + torch.log(two_pi))).sum()
